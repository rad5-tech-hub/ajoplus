...
const ProductCard = ({ product, onAddToCart }: ProductCardProps) => {
  ...
  return (
    <div className="bg-white border border-brand-200 rounded-3xl overflow-hidden hover:border-brand-400 hover:shadow-xl transition-all duration-300 group flex flex-col h-full">
      ...
      <div className="p-5 sm:p-6 flex flex-col flex-1">
        ...
        <h3 className="font-semibold text-base sm:text-lg leading-tight text-brand-900 mb-2 line-clamp-2">{product.name}</h3>
        <div className="text-2xl sm:text-3xl font-bold text-brand-600 mb-1">{formatNaira(product.price)}</div>
        {product.dollarPrice && (
        ...
        <button onClick={() => onAddToCart(product)} disabled={isOutOfStock}
          className="w-full bg-brand-600 hover:bg-brand-700 disabled:bg-brand-400 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-2xl flex items-center justify-center gap-2 transition-all duration-200 text-sm sm:text-base active:scale-[0.985] cursor-pointer">
