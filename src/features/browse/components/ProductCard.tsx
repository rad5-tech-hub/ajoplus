...
const ProductCard = ({ item }: ProductCardProps) => {
  ...
  return (
    <>
      <div className="bg-white border border-amber-200 rounded-3xl overflow-hidden hover:border-amber-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full">
        ...
        {!isPackage && (
          <div className="relative h-52 sm:h-56 md:h-60 lg:h-64 shrink-0 overflow-hidden">
            ...
            <div className="absolute top-4 right-4">
              <span className="bg-amber-600/90 backdrop-blur-sm text-white text-xs px-3.5 py-1 rounded-full font-medium shadow-sm">
                In Stock
              </span>
            </div>
          </div>
        ...
        <div className="p-5 sm:p-6 flex flex-col flex-1">
          {/* Category Tag */}
          <div className="inline-block px-4 py-1 bg-amber-100 text-amber-700 text-xs font-medium rounded-b-lg rounded-t-sm mb-3">
            {item.category}
          </div>

          {/* Title */}
          <h3 className="font-semibold text-base sm:text-lg leading-tight text-blue-950 mb-2 line-clamp-2 min-h-10">
            {item.title}
          </h3>

          {/* Price */}
          <div className="text-2xl sm:text-3xl font-bold text-amber-600 mb-4">
          ...
          {isPackage && item.progress !== undefined && (
            <div className="mb-5">
              <div className="flex justify-between text-xs mb-1.5">
                ...
                <span className="font-medium text-amber-600">{item.progress}%</span>
              </div>
          ...
          {isPackage && item.packageItems && (
            <div className="bg-slate-50 rounded-md border-b border-amber-200 sm:p-6">
              <h4 className="text-sm font-semibold text-blue-950 mb-3">Package Includes:</h4>
              <div className="max-h-48 overflow-y-auto">
                <ul className="space-y-2">
                  {item.packageItems.slice(0, 4).map((packageItem, index) => (
                    <li key={index} className="flex items-start gap-2 text-sm text-slate-700">
                      <span className="font-medium text-amber-600 hrink-0">{index + 1}.</span>
                      <span>{packageItem}</span>
                    </li>
                  ))}
                  {item.packageItems.length > 4 && (
                    <li className="flex items-start gap-2 text-sm text-slate-600 italic">
                      <span className="font-medium text-amber-600 shrink-0">+</span>
          ...
          <div className="mt-auto pt-2">
            <button
              ...
              disabled={isPending}
              className="w-full cursor-pointer bg-amber-600 hover:bg-amber-700 active:scale-[0.985] disabled:bg-amber-400 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-2xl flex items-center justify-center gap-2 transition-all duration-200 text-sm sm:text-base"
            >
