...
const MakePayment = () => {
    ...
    return (
      <div className="min-h-screen bg-[#f0f9f4] flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 rounded-full border-4 border-amber-200 border-t-amber-600 animate-spin mx-auto mb-4"></div>
    ...
    return (
      <div className="min-h-screen bg-[#f0f9f4] flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-blue-950 mb-2">Unable to Load Package</h2>
          <p className="text-slate-600 mb-6">
            {packageError instanceof Error ? packageError.message : 'Failed to load package details'}
          </p>
          <button
            onClick={() => navigate(-1)}
            className="bg-amber-600 text-white px-6 py-3 rounded-2xl font-semibold hover:bg-amber-700 transition-colors"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#f0f9f4]">
      {/* Header */}
      <div className="bg-[#f0f9f4] border-b border-amber-200 z-50">
        ...
        <div className="max-w-2xl mx-auto px-6 pb-6">
          <div className="flex items-center gap-4">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${step === 1 ? 'bg-amber-600 text-white' : 'bg-amber-100 text-amber-600'
              }`}>
              1
            </div>
            <div className="h-px flex-1 bg-slate-200"></div>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${step === 2 ? 'bg-amber-600 text-white' : 'bg-slate-200 text-slate-400'
              }`}>
              2
            </div>
          </div>
