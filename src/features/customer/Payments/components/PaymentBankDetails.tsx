...
const PaymentBankDetails = ({ onNext, totalAmount = 37500 }: PaymentBankDetailsProps) => {
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="bg-white rounded-3xl shadow-sm overflow-hidden border border-slate-100">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="font-semibold text-brand-900 mt-1">{bankDetails.bankName}</p>
          </div>
          <button
            onClick={() => copyToClipboard(bankDetails.bankName)}
            disabled={bankDetails.bankName === '—'}
            className="text-brand-600 cursor-pointer hover:text-brand-700 p-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="font-semibold text-brand-900 mt-1 font-mono tracking-wider">
          ...
          <button
            ...
            className="text-brand-600 hover:text-brand-700 p-2 cursor-pointer transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Copy className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 flex items-center justify-between">
          <div>
            ...
            <p className="font-semibold text-brand-900 mt-1">{bankDetails.accountName}</p>
          </div>
          <button
            onClick={() => copyToClipboard(bankDetails.accountName)}
            disabled={bankDetails.accountName === '—'}
            className="text-brand-600 hover:text-brand-700 p-2 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Copy className="w-5 h-5" />
          </button>
        </div>
      ...
      <button
        onClick={onNext}
        className="w-full cursor-pointer bg-brand-600 hover:bg-brand-700 active:bg-amber-800 transition-all text-white font-semibold py-4.5 rounded-2xl text-lg shadow-sm"
      >
        I've Made Payment
      </button>
    </div>
