...
const AjoDailySavings = ({ onOpenDailyModal }: AjoDailySavingsProps) => {
  ...
  return (
    <>
      <div className="bg-amber-600 text-white rounded-3xl p-5 w-full">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-amber-200 text-xs font-medium">Ajo Daily Savings</p>
            {isMultiPlan && (
              <p className="text-amber-300 text-xs mt-0.5">{planCount} active plans</p>
            )}
          </div>
          <span className="text-2xl">🐷</span>
        </div>
        ...
        <div className="bg-amber-50/20 border border-amber-100/30 py-3 px-4 rounded-2xl mb-3">
          <div className="space-y-2.5">
            ...
            <div>
              <p className="text-amber-100 text-xs">Commission Paid (5%)</p>
            ...
            <div className="pt-2 border-t border-amber-100/40">
              <p className="text-amber-100 text-xs">Available Balance</p>
              <p className="text-lg font-bold text-white">₦{(wallet?.availableBalance ?? availableBalance).toLocaleString()}</p>
            </div>
          </div>
        </div>

        <div className="bg-amber-50/20 border border-amber-100/30 py-3 px-4 rounded-2xl mb-4">
          <div className="space-y-2.5">
            <div>
              <p className="text-amber-100 text-xs">You've saved for</p>
              <p className="text-sm font-medium text-white">{wallet?.daysSaved ?? daysSaved} days</p>
            </div>
            <div>
              <p className="text-amber-100 text-xs">Next commission deduction</p>
              <p className="text-sm font-medium text-white">In 30 days</p>
            </div>
            <div>
              <p className="text-amber-100 text-xs">Monthly summary</p>
        ...
        <div className="flex flex-col gap-2">
          <button
            ...
            className="w-full cursor-pointer bg-white text-amber-700 font-semibold py-3 rounded-2xl hover:bg-amber-50 active:bg-amber-100 transition-colors text-sm"
          >
            Withdraw Balance
          </button>
          <button
