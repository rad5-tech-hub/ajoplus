    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between py-5 border-b border-slate-100 last:border-b-0 gap-3 sm:gap-0">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 text-lg
          ${credit ? 'bg-amber-100 text-amber-600' : 'bg-red-100 text-red-500'}`}>
          {credit ? '↑' : '↓'}
        </div>
      ...
      <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-end gap-2 shrink-0">
        <p className={`font-bold text-base sm:text-lg ${credit ? 'text-amber-600' : 'text-red-500'}`}>
...
const RecentTransactions = () => {
  ...
  if (error) {
    return (
      <div className="bg-white border border-red-100 rounded-3xl p-8 text-center shadow-sm">
        ...
        <button
          onClick={() => refetch()}
          className="text-amber-600 underline text-sm hover:text-amber-700 font-medium"
        >
          Retry
        </button>
      </div>
  ...
  if (transactions.length === 0) {
    return (
      <div className="bg-white border border-amber-200 rounded-3xl p-12 text-center shadow-sm">
        ...
        <h3 className="text-xl font-semibold text-blue-950 mb-3">No Transactions Yet</h3>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold px-8 py-3.5 rounded-2xl transition-all active:scale-[0.985]"
        >
          Browse Packages
  ...
  // ── Transactions list ──────────────────────────────────────────────────────
  return (
    <div className="bg-white border border-amber-200 rounded-3xl shadow-sm p-5 sm:p-6 w-full">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="font-semibold text-blue-950 text-base sm:text-lg">Recent Transactions</h3>
          <p className="text-xs text-slate-400 mt-0.5">{data?.count ?? 0} total</p>
        </div>
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-amber-600 disabled:opacity-40 transition-colors"
        >
