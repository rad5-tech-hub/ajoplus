    ...
    >
      <div className="flex items-start justify-between mb-6">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              ...
              {pkg.status === 'finalised' && <span className="w-2 h-2 bg-brand-500 rounded-full mr-1.5" />}
              {pkg.status === 'finalised' ? 'Finalised' : pkg.status === 'completed' ? 'Completed' : pkg.status.charAt(0).toUpperCase() + pkg.status.slice(1)}
            </span>
          </div>
        ...
        <div className="text-right shrink-0 ml-4 flex items-center gap-2">
          <div>
            <p className={`text-2xl font-bold ${isInactive ? 'text-slate-400' : 'text-brand-600'}`}>{amount}</p>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6 bg-slate-50 p-4 rounded-2xl">
        <div>
          ...
          <p className={`font-semibold mt-1 ${isInactive ? 'text-slate-500' : 'text-brand-900'}`}>{totalPaid}</p>
        </div>
        <div>
          <p className="text-xs text-slate-400">Remaining</p>
          <p className={`font-semibold mt-1 ${isInactive ? 'text-slate-400' : pkg.remainingBalance === 0 ? 'text-brand-600' : 'text-red-600'}`}>
            {remaining}
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-400">Progress</p>
          <p className={`font-semibold mt-1 ${isInactive ? 'text-slate-500' : 'text-brand-600'}`}>{pkg.progress}%</p>
        </div>
      </div>

      <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden mb-4">
        <div
          className={`h-full rounded-full transition-all duration-300 ${isInactive ? 'bg-slate-300' : 'bg-brand-600'}`}
      ...
      <div className="flex items-center justify-between text-sm text-slate-400">
        ...
        {pkg.status === 'active' && <div>Next due: {nextDue}</div>}
      </div>

      {(pkg.status === 'finalised' || pkg.status === 'completed') && pkg.claimCode && (
        <ClaimCodeSection
