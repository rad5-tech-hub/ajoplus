  ...
  return (
    <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
      {cards.map(({ label, sub, value, icon: Icon }) => (
        <div
          key={label}
          className="bg-white p-4 sm:p-5 lg:p-6 rounded-2xl sm:rounded-3xl border border-amber-200/50 shadow-md flex flex-col gap-4"
        >
          {/* Header row */}
          <div className="flex justify-between items-start gap-2">
            ...
            <div className="w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 bg-amber-100 rounded-xl sm:rounded-2xl flex items-center justify-center shrink-0">
              <Icon className="w-4 h-4 lg:w-5 lg:h-5 text-amber-600" />
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <p className="text-2xl sm:text-3xl lg:text-4xl font-bold text-blue-950 leading-none">
              {value}
            </p>
          )}
        </div>
