...
const OverviewCards = () => {
  ...
  const Card = ({ label, sub, value, icon: Icon }: { label: string; sub: string; value: string; icon: React.ElementType }) => (
    <div className="bg-white p-4 sm:p-5 lg:p-6 rounded-2xl sm:rounded-3xl border border-brand-200/50 shadow-md flex flex-col gap-4">
      ...
      {isLoading ? (
        ...
        <p className="text-2xl sm:text-3xl lg:text-4xl font-bold text-brand-900 leading-none">{value}</p>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
        {primaryCards.map((c) => <Card key={c.label} {...c} />)}
      </div>

      {/* Toggle button */}
      <button
        onClick={() => setShowAll(!showAll)}
        className="mt-4 w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl border border-brand-200/60 text-sm font-medium text-slate-500 hover:text-brand-700 hover:bg-brand-50/50 hover:border-brand-300 transition-all duration-200 cursor-pointer"
      >
        <span>{showAll ? 'Show fewer metrics' : 'Show all metrics'}</span>
        <ChevronDown className={cn('w-4 h-4 transition-transform duration-300', showAll && 'rotate-180')} />
      </button>

      {/* Collapsible row */}
      <div className={cn(
        'grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 overflow-hidden transition-all duration-300',
        showAll ? 'mt-3 sm:mt-4 lg:mt-6 opacity-100' : 'mt-0 opacity-0 pointer-events-none max-h-0'
      )}>
        {moreCards.map((c) => <Card key={c.label} {...c} />)}
      </div>
    </div>
  );
};
