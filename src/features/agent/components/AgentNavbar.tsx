...
import { LogOut, User, Menu, X, UserCog, ShoppingCart } from 'lucide-react';
import { useAuthStore } from '@/app/store/authStore';
import { useCartStore } from '@/app/store/CartStore';

const NAV_LINKS = [

  { label: 'Dashboard', to: '/dashboard/agent', icon: UserCog },
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          ...
          {/* Logo */}
          <Link to="/dashboard/agent" className="flex items-center gap-2.5 shrink-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 bg-brand-600 text-white rounded-xl sm:rounded-2xl flex items-center justify-center font-bold text-base sm:text-lg leading-none">
              A+
            </div>
            <span className="font-bold text-[18px] sm:text-[20px] tracking-tight text-brand-900 hidden xs:block">
              AbaGold
            </span>
          </Link>
