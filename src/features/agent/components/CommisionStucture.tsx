...
const CommissionStructure = () => {
  return (
    <div className="bg-brand-50 border border-brand-100 rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-8">
      <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-brand-900 mb-4 sm:mb-5 lg:mb-6">
      ...
      <div className="divide-y divide-brand-100">
        {rows.map(({ label, value }) => (
          <div key={label} className="flex justify-between items-center py-3 sm:py-3.5 lg:py-4 first:pt-0 last:pb-0">
            ...
            <span className="text-sm sm:text-[15px] font-semibold text-brand-700">{value}</span>
          </div>
        ))}
      </div>
    </div>
