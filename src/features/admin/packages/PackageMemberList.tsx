			...
			<div className="max-w-6xl mx-auto px-4 sm:px-6">
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
					<button
						type="button"
						onClick={() => navigate(-1)}
						className="inline-flex items-center gap-2 text-slate-600 hover:text-blue-950 bg-white border border-amber-200 rounded-2xl px-4 py-2 transition-colors"
					>
						<ChevronLeft className="w-4 h-4" /> Back
					</button>
					<div className="space-y-1">
						<h1 className="text-2xl sm:text-3xl font-bold text-blue-950">{headerLabel}</h1>
						<p className="text-sm text-slate-500">Member list and package revenue details</p>
					</div>
					<div className="rounded-3xl border border-amber-200 bg-white p-5 text-center min-w-50">
						<p className="text-sm text-slate-500">Total Revenue</p>
						<p className="mt-2 text-2xl font-bold text-amber-600">{formatCurrency(totalRevenue)}</p>
					</div>
				</div>
				...
				{isLoading && (
					<div className="space-y-4">
						{[1, 2, 3].map((item) => (
							<div key={item} className="rounded-3xl border border-amber-200 bg-white p-6 animate-pulse space-y-4" />
						))}
					</div>
				)}
				...
				<div className="space-y-4">
					{members.map((member) => (
						<div key={member.id} className="rounded-3xl border border-amber-200 bg-white p-6 shadow-sm">
							<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
								<div className="flex items-center gap-4">
									<div className="h-14 w-14 rounded-2xl bg-amber-100 text-amber-700 grid place-items-center font-semibold text-lg">
									...
									<div>
										<p className="text-lg font-semibold text-blue-950">{member.user.fullName}</p>
							...
							<div className="mt-5 grid gap-4 sm:grid-cols-3">
								<div className="rounded-3xl bg-slate-50 p-4">
									...
									<p className="mt-2 text-lg font-semibold text-blue-950">{formatCurrency(member.totalPaid)}</p>
								</div>
								<div className="rounded-3xl bg-slate-50 p-4">
									<p className="text-sm text-slate-500">Remaining</p>
									<p className="mt-2 text-lg font-semibold text-blue-950">{formatCurrency(member.remainingBalance)}</p>
								</div>
								<div className="rounded-3xl bg-slate-50 p-4">
									<p className="text-sm text-slate-500">Progress</p>
									<p className="mt-2 text-lg font-semibold text-blue-950">{member.progress}%</p>
								</div>
							</div>

							<div className="mt-5">
								<div className="h-2 bg-slate-100 rounded-full overflow-hidden">
									<div className="h-full rounded-full bg-amber-600" style={{ width: `${Math.min(Math.max(member.progress, 0), 100)}%` }} />
								</div>
								<div className="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
									...
									{member.status === 'completed' && (
										<button
											...
											className="rounded-2xl bg-amber-600 text-white px-4 py-2 text-sm font-semibold hover:bg-amber-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
										>
