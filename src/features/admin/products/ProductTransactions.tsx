			...
			<div className="max-w-6xl mx-auto px-4 sm:px-6">
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
					...
					<div className="space-y-1">
						<h1 className="text-2xl sm:text-3xl font-bold text-blue-950">{productName}</h1>
						<p className="text-sm text-slate-500">All transactions for this product</p>
					</div>
					<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
						<div className="rounded-3xl border border-amber-200 bg-white p-5 text-center">
						...
						<div className="rounded-3xl border border-amber-200 bg-white p-5 text-center">
							...
							<p className="mt-2 text-2xl font-bold text-blue-950">{headerSubtitle}</p>
						</div>
					</div>
				</div>
				...
				{isLoading && (
					<div className="space-y-3">
						{[1, 2, 3, 4].map((index) => (
							<div key={index} className="rounded-3xl border border-amber-200 bg-white p-5 animate-pulse h-28" />
						))}
					</div>
				)}
									...
									<tr key={transaction.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
										<td className="py-5 px-5">
											<div className="flex items-center gap-3">
												<div className="w-10 h-10 rounded-2xl bg-amber-100 text-amber-700 grid place-items-center font-semibold">
												...
												<div>
													<p className="font-semibold text-blue-950">{transaction.user.fullName}</p>
												</div>
											</div>
										</td>
										<td className="py-5 px-5 text-slate-500 text-sm">{transaction.user.email}</td>
										<td className="py-5 px-5 font-semibold text-amber-600">{formatCurrency(transaction.amount)}</td>
