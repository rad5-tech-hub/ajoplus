			...
			<div className="max-w-6xl mx-auto px-4 sm:px-6">
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
					...
					<div className="space-y-1">
						<h1 className="text-2xl sm:text-3xl font-bold text-slate-900">{productName}</h1>
					...
					<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
						...
						<div className="rounded-3xl border border-slate-200 bg-white p-5 text-center">
							...
							<p className="mt-2 text-2xl font-bold text-slate-900">{headerSubtitle}</p>
						</div>
					</div>
				</div>
				...
				{isLoading && (
					<div className="space-y-3">
						{[1, 2, 3, 4].map((index) => (
							<div key={index} className="rounded-3xl border border-slate-200 bg-white p-5 animate-pulse h-28" />
						))}
					</div>
				)}
									...
									<tr key={transaction.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
										<td className="py-5 px-5">
											<div className="flex items-center gap-3">
												<div className="w-10 h-10 rounded-2xl bg-emerald-100 text-emerald-700 grid place-items-center font-semibold">
												...
												<div>
													<p className="font-semibold text-slate-900">{transaction.user.fullName}</p>
												</div>
											</div>
										</td>
