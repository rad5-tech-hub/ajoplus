        ...
        <tbody>
          {submissions.map((sub) => (
            <tr key={sub.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
              <td className="py-5 px-5 font-medium text-slate-900">{sub.fullName}</td>
              <td className="py-5 px-5 text-slate-500 text-sm">{sub.email}</td>
              <td className="py-5 px-5 text-emerald-600 font-semibold">{formatCurrency(sub.amount)}</td>
              ...
              <td className="py-5 px-5">
                <a href={sub.proofUrl} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-emerald-600 hover:text-emerald-700 text-xs underline">
                ...
                <td className="py-5 px-5">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="p-2 bg-emerald-100 text-emerald-600 hover:bg-emerald-200 rounded-xl transition-colors disabled:opacity-50"
                      title="Approve"
                    >
                  ...
                  {rejectId === sub.id && (
                    <div className="mt-3 flex gap-2">
                      <input
                        ...
                        className="flex-1 px-3 py-1.5 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-red-400"
                      />
                      <button
    ...
    <div className="space-y-6">
      <div className="flex gap-4 border-b border-slate-200">
        <button
          ...
          className={`pb-3 font-medium text-sm transition-colors border-b-2 ${activeTab === 'pending' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-500'
            }`}
        >
          Pending {pending.length > 0 && <span className="ml-1.5 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-xs">{pending.length}</span>}
        </button>
        <button
          onClick={() => setActiveTab('history')}
          className={`pb-3 font-medium text-sm transition-colors border-b-2 ${activeTab === 'history' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-500'
            }`}
        >
          History
        </button>
