  ...
  if (isError) {
    return (
      <div className="bg-white border border-red-200 rounded-3xl p-8 text-center">
        ...
        <button onClick={() => refetch()} className="px-5 py-2 rounded-2xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 transition-colors cursor-pointer">
          Retry
        </button>
      </div>
    );
              ...
              {fees.map((fee) => (
                <tr key={fee.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="py-5 px-5">
                    <p className="font-medium text-slate-900">{fee.fullName || '—'}</p>
                    <p className="text-xs text-slate-400">{fee.email || ''}</p>
                  </td>
                  <td className="py-5 px-5 font-semibold text-emerald-600">{formatCurrency(fee.amount)}</td>
                  ...
                  <td className="py-5 px-5">
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-emerald-600 hover:text-emerald-700 text-xs underline cursor-pointer"
                    >
                  ...
                  <td className="py-5 px-5">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="p-2 bg-emerald-100 text-emerald-600 hover:bg-emerald-200 rounded-xl transition-colors disabled:opacity-50 cursor-pointer"
                        title="Approve"
                      >
