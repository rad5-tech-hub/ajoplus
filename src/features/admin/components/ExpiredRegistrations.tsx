  ...
  if (isLoading) {
    return (
      <div className="space-y-3">
        {[1, 2, 3].map((i) => <div key={i} className="h-16 bg-slate-100 rounded-3xl animate-pulse" />)}
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {[
          ...
          { value: 'customer' as RoleFilter, label: 'Customers' },
          { value: 'agent' as RoleFilter, label: 'Agents' },
        ].map(({ value, label }) => (
          <button key={value} onClick={() => handleFilterChange(value)}
            className={`px-3 py-1.5 rounded-2xl text-xs font-medium transition-colors cursor-pointer
              ${roleFilter === value ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
            {label}
          </button>
        ))}
      </div>

      <p className="text-sm text-slate-500">Showing {filtered.length} expired user{filtered.length !== 1 ? 's' : ''}</p>
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="py-4 px-5 text-sm font-semibold text-slate-500">Role</th>
                ...
                {filtered.map((user) => {
                  ...
                  return (
                    <tr key={user.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-5 px-5 text-sm text-slate-500 capitalize">{user.role || '—'}</td>
            ...
            <div className="flex items-center justify-between bg-white border border-brand-200 rounded-2xl px-5 py-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
                  className="px-4 py-2 border border-brand-200 rounded-2xl text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">Previous</button>
                <button onClick={() => setPage((p) => Math.min(meta.totalPages, p + 1))} disabled={page >= meta.totalPages}
                  className="px-4 py-2 border border-brand-200 rounded-2xl text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">Next</button>
              </div>
            </div>
          )}
        </>
